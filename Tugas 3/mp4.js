...
var sphereColors = [
  0.8, 0.8, 0.8, 1.0, 0.8, 0.0, 1.0, 0.0, 0.3, 0.17, 0.88, 0.0,
]; //colors of each type (silver, gold, pink, grn)
var sphereColors_string = [
  "color:#cccccc",
  "color:#ffcc00",
  "color:#ff004c",
  "color:#2ce000",
...
function setupShaders() {
  ...
  shaderProgram.fs_mvMatrixUniform = gl.getUniformLocation(
    shaderProgram,
    "fs_uMVMatrix"
  );
  ...
  shaderProgram.uniformLightPositionLoc = gl.getUniformLocation(
    shaderProgram,
    "uLightPosition"
  );
  shaderProgram.uniformAmbientLightColorLoc = gl.getUniformLocation(
    shaderProgram,
    "uAmbientLightColor"
  );
  shaderProgram.uniformDiffuseLightColorLoc = gl.getUniformLocation(
    shaderProgram,
    "uDiffuseLightColor"
  );
  shaderProgram.uniformSpecularLightColorLoc = gl.getUniformLocation(
    shaderProgram,
    "uSpecularLightColor"
  );
  shaderProgram.fogUniform = gl.getUniformLocation(shaderProgram, "fog");
  shaderProgram.terrainUniform = gl.getUniformLocation(
    shaderProgram,
    "terrain"
  );
}
...
function setupBuffers() {
  ...
  gl.bufferData(
    ...
    new Uint16Array(terrainIndices),
    gl.STATIC_DRAW
  );
...
function drawTerrain(rotX, rotY, rotZ) {
  ...
  mat4.scale(
    mvMatrix,
    mvMatrix,
    vec3.fromValues(terrainScale[0], terrainScale[1], terrainScale[2])
  );
  ...
  for (var i = 0; i < sphereLists.length; i++) {
    ...
    while (curSphere != null) {
      drawMesh(
        sphereVertexBuf,
        sphereNormalBuf,
        sphereColors.slice(3 * i, 3 * i + 3),
        curSphere.position,
        0.05
      ); //silver spheres (normal score)
...
function handleKeys() {
  ...
  if (pressedKeys[37] || pressedKeys[65]) {
    //Left arrow OR A: plane turn left
  ...
  if (pressedKeys[40] || pressedKeys[83]) {
    //Down arrow OR S: pitch down
    var axis = normalize(cross(viewDir, up));
    quat.setAxisAngle(
      eyeQuatUD,
      axis,
      degToRad(turnFactor * -1.0 * rotDegrees)
    ); //create the quat
    vec3.transformQuat(viewDir, viewDir, eyeQuatUD); //apply to viewDir
    vec3.transformQuat(up, up, eyeQuatUD); //apply to up (so viewDir and up are always perpendicular)
  }
  ...
  if (pressedKeys[187] || pressedKeys[49]) {
    // =/+ OR 1: Increase speed
    if (speed < maxSpeed) speed += speedIncrement;
  }
  if (pressedKeys[189] || pressedKeys[51]) {
    // - OR 3: Decrease speed
...
function showMessages() {
  if (msg_info_timeLeft > 0) {
    if (doublePoints) {
      //hit a green sphere: Display perks and countdown timers
      var msgStr =
        "Double Points (" +
        Math.ceil(msg_info_timeLeft / 1000).toString() +
        ")! ";
      if (msg_info_timeLeft < 10000) {
        reverseGravity = false; //reverse gravity expired
        msg_info_node.nodeValue = msgStr;
      } else {
        //still have reverse gravity
        msg_info_node.nodeValue =
          msgStr +
          "Reverse Gravity (" +
          Math.ceil((msg_info_timeLeft - 10000) / 1000).toString() +
          ")!";
      }
    } else {
      //hit a pink sphere: warning message
...
function startup() {
  ...
  for (var i = 0; i < sphereLists.length; i++) {
    //set colors of some of the instructions text
    document.getElementById("text_sphere" + i.toString()).style =
      sphereColors_string[i];
  }
...
function tick() {
  ...
  if (!paused) {
    ...
    if (collisions[2] > 0) {
      //Hit bad sphere: lose all points!
      if (score > 0) {
        //don't want to show "-0" if score already 0
      ...
      if (!doublePoints) {
        //only display warning message if double points/reverse gravity message not already showing
      ...
      if (collisions[0] > 0) {
        //hit silver
        msg_score_node.nodeValue =
          "+" + (multiplier * collisions[0]).toString();
      ...
      if (collisions[1] > 0) {
        //hit gold
        msg_score_node.nodeValue =
          "+" + (multiplier * collisions[1] * 3).toString();
