...
class SphereList {
  ...
  delete(previous, current) {
    //delete (current), but need (previous) for bookkeeping.
...
function createSpheres(
  ...
  if (typeof sphereList === "undefined" || sphereList == null)
    sphereList = new SphereList();
  ...
  for (var i = 0; i < numSpheres; i++) {
    ...
    for (var p = 0; p < 3; p++) {
      position[p] = randFloatInRange(
        positionBounds[2 * p],
        positionBounds[2 * p + 1]
      );
    }
    //Generate current sphere initial velocity.
    for (var v = 0; v < 3; v++) {
      velocity[v] = randFloatInRange(
        velocityBounds[2 * v],
        velocityBounds[2 * v + 1]
      );
    }
...
function updatePVA(
  ...
  while (curSphere != null) {
    ...
    for (var i = 0; i < 3; i++) {
      curSphere.velocity[i] =
        curSphere.velocity[i] * dragTime +
        accelMultiplier * curSphere.acceleration[i] * timeElapsed;
    }
    prev = curSphere;
    curSphere = curSphere.next;
  }
...
function collisionDetection(sphere, colliderPosition, tolerance) {
  for (var i = 0; i < 3; i++) {
    if (
      colliderPosition[i] < sphere.position[i] - sphere.radius - tolerance ||
      colliderPosition[i] > sphere.position[i] + sphere.radius + tolerance
    ) {
